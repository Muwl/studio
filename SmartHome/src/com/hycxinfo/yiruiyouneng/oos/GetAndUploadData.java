package com.hycxinfo.yiruiyouneng.oos;
...
/**
 * @author: zhouzhuo<yecan.xyc@alibaba-inc.com> Apr 2, 2015
 * 
 */
public class GetAndUploadData {
	...
	public GetAndUploadData(Context context, Handler handler) {
		super();
		this.context = context;
		this.handler = handler;
		ossService = OSSAndroid.ossService;
		bucket = ossService.getOssBucket(OSSAndroid.bucketName); // 替换为你的bucketName
		bucket.setBucketHostId("oss-cn-beijing.aliyuncs.com");
	}

	public void show() {

		// bucket = ossService.getOssBucket("demoBucket");
	...
	// 异步上传数据
	public void asyncUpload(String temp) {
		byte[] dataToUpload = temp.getBytes();
		OSSData data = ossService.getOssData(bucket,
				ShareDataTool.getGateId(context));
		data.setData(dataToUpload, "text/plain");
...
//						+ " bytes: " + byteCount + " in total: " + totalSize);
			}
...
//				Log.d(TAG, "[onSuccess] - " + objectKey + " length: "
//						+ data.length);
...
//					ToastUtils.displayShortToast(context, "下载成功！");
...
//				Log.d(TAG, "[onProgress] - current download: " + objectKey
//						+ " bytes:" + byteCount + " in total:" + totalSize);
			...
			@Override
			public void onFailure(String objectKey, OSSException ossException) {
				Log.e("lllllllllllllll", "jjjjjjjjjjjjjjjjjjj");
				handler.sendEmptyMessage(-999);
//				Log.e(TAG, "[onFailure] - download " + objectKey + " failed!\n"
//						+ ossException.toString());
//				ToastUtils.displayShortToast(context, "下载失败！");
			}
		});
	}
	...
	public void getMetaAfterGetData() {
		...
		try {
			...
			for (BasicNameValuePair pair : metaList) {
				Log.d(TAG, "[getMetaAfterGetData] - " + pair.getName() + " : "
						+ pair.getValue());
			}
		} catch (OSSException e) {
			HandleException.handleException(e);
